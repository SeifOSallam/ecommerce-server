...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if rate_gt:
            queryset = queryset.annotate(avg_rate=Avg('rate__rate')).filter(avg_rate__gt=float(rate_gt))
        
        if rate_lt:
            queryset = queryset.annotate(avg_rate=Avg('rate__rate')).filter(avg_rate__lt=float(rate_lt))
        
        #product/?price_gt=1&price_lt=4
        if price_gt:
            queryset = queryset.annotate(avg_price=Avg('price')).filter(avg_price__gt=float(price_gt))
        
        if price_lt:
            queryset = queryset.annotate(avg_price=Avg('price')).filter(avg_price__lt=float(price_lt))
    ...
    def popular_products(self, request):
        queryset = self.get_queryset().annotate(total_rates=Count('rate')).order_by('-total_rates')
